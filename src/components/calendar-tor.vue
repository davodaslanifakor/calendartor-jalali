          ...
          <div class="header-calendar" :style="fixHeader">
            <div class="row no-gutters">
                ...
                <div class="col-sm-4" v-if="false">
                  <div class="action-calendar">
                    <label> نمایش  از {{weeks[0].timestamp  | Dialect}} تا {{weeks[weeks.length - 1].timestamp  | Dialect}}</label>
                    <button class="mdi mdi-plus" v-ripple="'rgba(15, 72, 128, 0.57)'" v-tooltip="'اضافه کردن'" @click="pluseWeek"></button>
                    <button class="mdi mdi-minus" v-ripple="'rgba(15, 72, 128, 0.57)'" v-tooltip="'کم کردن'" @click="minusWeek"></button>
                  </div>
                </div>
                 <div class="col-sm-3">
                ...
                <div class="col-sm-3 offset-4">
                  <div class="action-calendar text-left">
                    <!-- deActive -->
                    <button class="mdi mdi-arrow-right" v-if="weeks && weeks.length > 0" :class="{'deActive' :  weeks[0].timestamp < now}" v-ripple="'rgba(15, 72, 128, 0.57)'" v-tooltip="'هفته قبل'" @click="pervWeek"></button>
      ...
      <div class="row no-gutters">
          <div v-for="(week,index) in weeks" :key="index" class="cols col-sm column" ref="cols">
            ...
            <div v-for="(item,i) in count" :key="i" :style="{height:scale+'px'}" v-if="(startTime + i * interval) < 24 * 60 " :class="['minute',{'border':item%2==0},{'before-time':(startTime * 60  + week.timestamp + i * interval * 60) < now }]" >
                ...
                <div :class="['box-event']" v-for="(visit,index) in events[week.timestamp.valueOf() + (startTime + i  * interval)*60]" :key="index + i"
                 :style="{height: (visit.visitType.duration/interval)*scale+'px','backgroundColor':visit.visitType.rgb}"
...
export default {
  ...
  methods: {
    ...
    pervWeek() {
      if(this.weeks[0].timestamp > this.now){
